...
window.addEvent('domready', function() {
	...
	$('frame_view').addEvent('load', function() {
		ShowFrameView();
		$cwe.postNotify('ChargeWndShowed');
	});

	/*! @brief [在页面完全加载后才显示出来] */
	$cwe.addNotify('cweui.chargeWndStateChange', function(state) {
		if(state == 'show') {
...
function ShowInfomation(divId, adUrlName, ifurlName) {
	$funCtrl.getFunCfg(FUN_SEARCHCHARGE, function(cfg) {
		if(cfg == null) return;
		if(cfg[FUN_SEARCHCHARGE + '_attributes'] == undefined) return;
		if(cfg[FUN_SEARCHCHARGE + '_attributes'][adUrlName] == undefined) return;
		if(cfg[FUN_SEARCHCHARGE + '_attributes'][ifurlName] == undefined) return;
		var url = cfg[FUN_SEARCHCHARGE + '_attributes'][adUrlName];
		if(url == '') return;

		var ifurl = cfg[FUN_SEARCHCHARGE + '_attributes'][ifurlName];
		if(ifurl == '') return;
