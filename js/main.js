var wWeather=false, wIsEnterWnd=false, m_isPreConnected=false, m_isConnected=false, m_isAccountWnd=false;
var svrMgr=new SvrMgr();
var dialMgr=new DialMgr();
var m_gFunJson={"0":"InforURL", "1":"ChargeUrl","2":"FreeAccountUrl","3":"SpeedUp","4":"ForgotPassword","5":"ChatUrl"}; //配置在此处的功能，html中的id必须要与此名字保持一致
var needStop = { /*'MktIntf': '0'*/};
...
function init(){
	...
	App.start(function(flag){
		...
		$infoCenter.setConfigValue('OtherSetting.showRemindInfo', '0', false, function() {});
...
function UIMsgReg(){
    ...
    $passwordInput=new InputEx('txt_passwd','请输入密码',true,'pw', onKeyUpEvent);

	$('net').addEvent('click', function() {//网络连接界面
	    AccountInfoToNetInfo();
	});

	$('account').addEvent('click', function() {//我的账号界面
	    NetInfoToAccoutnInfo();
        SetClickModule(3, CLICK_MYACCOUNT);
	});

    $('AccountInfo').addEvent('load', function() {//等待我的账户url加载完成才显示
        $('loading_account').setStyle('display', 'none');
        $('AccountInfo').setStyle('display', 'block');
    });
	
	$('InforURL').addEvent('click', function(){//资讯
        //App.openWindow('info',true,'thread', 'info.html', '', function() { });
        $InfoWnd.ShowInfoWnd();
        SetClickModule(3, CLICK_INFORURL);
	});
    $cwe.addNotify('cweui.InfoWndShowed', function() {
        $InfoWnd.ShowInst();
    });
    
    //添加账单查询界面打开监听 zlm 2020.04.08
    $cwe.addNotify('cweui.BillWndShowed', function() {
    	$BillWnd.ShowInst();
    });
    
    //添加充值缴费查询界面打开监听 zlm 2020.04.09
    $cwe.addNotify('cweui.ChargeWndShowed', function() {
    	$ChargeWnd.ShowInst();
    });

	$('login_qr_button').addEvent('click', function(){//切换到账号登录界面按钮
        m_isAccountWnd=true;
		login_view('login_qr','login_pad');
		//login_view('login_qr','login_complete');
	});
	$('login_pad_button').addEvent('click', function() {//切换到二维码界面按钮
	    if (App.eState == $ESTATE.ES_CONNECTING) return;
	        login_view('login_pad', 'login_qr');
	});
	$('chk_save_pwd').addEvent('click', function(){//记住密码
    ...
    $('SpeedUp').addEvent('click', function() {
        App.openWindow('AboutWnd',true,'thread','AKeyToSpeedUp.html', '', function() { });
    });
	
    $('ChargeUrl').addEvent('click', function(){//充值缴费
        OpenUrlWithParam(FUN_CHARGEURL, 'url');
        SetClickModule(3, CLICK_CHARGEURL);
    });
    $('FreeAccountUrl').addEvent('click', function(){//异网体验
    ...
    $('handled').addEvent('click', function() {//掌上大学
    ...
    $('ShowOrHidePwd').addEvent('click', function() {
    ...
    $('NetCheckRefresh').addEvent('click', function() {
        login_view(App.currentView, 'login_checkstate');
        dialMgr.start();
    });
    
    $('NetCheckHelp').addEvent('click', function() {
        $cwe.plugin('IOUtil').call('OpenURL', '\\ErrorView\\ErrorInfo.html',function() { });
    });
    
    $('NetCheckTool').addEvent('click', function() {
        OpenCheckAndRepaireTool();
        //App.openWindow('DetectionAndRepairTool', true, 'thread', 'DetectionAndRepairTool.html', '', function() { });
    });

    $cwe.addNotify('cweui.openchangepassword', function() {
        OpenUrlWithParam(FUN_PASSWORDURL, 'url', 'action=modifyPassword');
    });

    $cwe.addNotify('cweui.moduleclickmsg', function(adid) {
        uploadADClickInfo(adid);
    });

    $('ChatUrl').addEvent('click', function(){
        App.openWindow('SmarkChat',true,'thread','SmartChat.html', '', function() { });
...
function ReceiveMsg() {
    //接收右下角图标的响应信息
    $cwe.addNotify('cweui.shellnotify.lbuttonup', function() {
        $cwe.feature('status:max', function() { });
    });

    $cwe.addNotify('cweui.shellnotify.rbuttonup', function(x, y) {
        $cwe.popupMenu($MenuObject.IconMenuJson, x, y, function(SelectedItem) {
    ...
    $cwe.addNotify('cweui.MainWnd', function(isCloseClient) {
        if (isCloseClient) {
            CloseOperation();
        }
        else {
    ...
    $cwe.addNotify('cweui.AKeySpeedUp', function(isOpenSpeedIcon) {
        if (isOpenSpeedIcon) {
    ...
    $cwe.addNotify('cweui.accountcallback', function() {
        ShowLoadingOrNormal('loading_account', 'AccountInfo', 2);
    });

    $cwe.addNotify('cweui.opensettingwnd', function() {
...
window.addEvent('load', function() {  
    ...
    $cwe.shellNotifyIcon({ tip: '天翼校园', icon: 128 }, function() { });
});
...
function UpdateComplete(svrMsgObj){
    ...
    ShowLastError('', 50000005, '', MB_OKCANCEL, function(paramJson){
...
function handlePreConnectedMsg(svrMsgObj){
    ...
    $MenuObject.createMenuCfg($MenuObject.itemPreConnStatus,function(){item});
    ...
    if(App.currentView == 'login_checkstate') {
        //判断后台配置的登录方式
        $infoCenter.getMulValue('global', 'authType', 'authDefautType',function(values){
            if(values == undefined)
            {
    ...
    //set account
    $infoCenter.getConfigValue('Account.Portal', false, function(value){
        $userInput.setValue(value);
        $infoCenter.setGlobalValue('account', value, function(){});
        mAccount = value;
    });
    $infoCenter.getConfigValue('Account.Save', false, function(value){
        if(value == '0'){
            $('chk_save_pwd').checked = false;  
        }else{
            $('chk_save_pwd').checked = true;
            $infoCenter.getConfigValue('Account.PortalPassword', true, function(value){
   ...
   $funCtrl.GetState("InforURL", "enable", function(funName, cfg) {
	   if(cfg != null) {
		   
		   $infoCenter.setConfigValue('OtherSetting.showRemindInfo', '1', false, function() {});
		   
		   // zhangliangming 2020.03.11 添加资讯点击量上报的监听事件
	        $cwe.addNotify('cweui.InfoModuleClick', function(param) {
	        ...
	        $cwe.addNotify('cweui.InfoUpdateLastDate', function(param) {
	        	var paramJson = JSON.decode(param);
		   ...
		   $infoCenter.getConfigValue('OtherSetting.RemindInfo', false, function(isRemindInfoStart) {
			   if(isRemindInfoStart == '1') {
				   $infoCenter.getConfigValue('OtherSetting.RemindDate', false, function(remindDate) {
					   if(remindDate != '' && remindDate != undefined && remindDate == GetLocalData()){
						   return;
						   }
					   $infoCenter.setConfigValue('OtherSetting.RemindDate', GetLocalData(), false, function() { });
			...
			JsonPostHTTP(ifurl, {
				...
				//alert(responseText);
                 ...
                 if (response != undefined && response.code == 0) {
                	 ...
                	 if (allPageData == null || allPageData == '' || allPageData == undefined || allPageData.length == undefined || allPageData.length == 0) {
                	 } else {
...
function handleInfoPageData(allPageData){
	$infoCenter.getConfigValue('OtherSetting.InfoLastDate', false, function(localInfoLastDate) {
...
function handleConnectedMsg(svrMsgObj) {
    ...
    $infoCenter.getMulValue('global', 'authType', 'authDefautType',function(values) {
        ...
        if (opParam.type == $ETYPE.ET_UNKNOWN) {
            ...
            $infoCenter.setConfigValue('Account.Save', save, false, function () {});
...
function ticketResp(svrMsgObj){
	...
	$cwe.plugin('ioutil').call('CreateQRIamge',qrImage,opParam.qr, function(ret){
		if(ret){
			$('login_qr_image').src='';
...
function DoLoginPAP() {
    ...
    //保存账号密码到本地
    $infoCenter.setConfigValue('Account.Portal', userid, false, function(){});
    $infoCenter.setConfigValue('Account.PortalPassword', passwd, true, function(){});

    mAccount = userid;
    var nowTime = GetLocalData();
    $infoCenter.getConfigValue('ClickUploadTime', nowTime, function(recordTime){
...
function GetSpeedAndFlow(){
    $cwe.plugin('NetwordUtil').call('GetCurNetSpeedAndFlow',function(UpLoadSpeed, UpLoadTotal, DownLoadSpeed, DownLoadTotal){
...
function DealCloseEvent() {
    $infoCenter.getConfigValue('SystemSetting.Prompt', false, function(isSaveClose) {
        if (isSaveClose === '1') {
            $infoCenter.getConfigValue('SystemSetting.Close', false, function(MinOrExit) {
                if (MinOrExit == '0') {
                    $cwe.feature('status:hide', function() { });
                }
                else {
            ...
            $cwe.open('modal', 'DlgExit.html', '', function() {
            });
        }
    });
}
...
{
    $cwe.plugin('IOUtil').call('CloseExeByName', '', 'SelfDebugTool',function() { });
    $cwe.feature('status:hide', function() { });
    $cwe.shellNotifyIcon({}, function() { });
    WriteClickModuleData(mClickModuleJson);
    dialMgr.disconnect($DIS_REASON.DIS_REASON_MANUAL,function(){
        //closeEvent();
        function delayClose(){
            Stop();
