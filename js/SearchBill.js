...
window.addEvent('domready', function() {
	...
	$('frame_view').addEvent('load', function() {
		ShowFrameView();
		$cwe.postNotify('BillWndShowed');
	});

	/*! @brief [在页面完全加载后才显示出来] */
	$cwe.addNotify('cweui.billWndStateChange', function(state) {
		if(state == 'show') {
...
function ShowInfomation(divId, adUrlName, ifurlName) {
	$funCtrl.getFunCfg(FUN_SEARCHBILL, function(cfg) {
		if(cfg == null) return;
		if(cfg[FUN_SEARCHBILL + '_attributes'] == undefined) return;
		if(cfg[FUN_SEARCHBILL + '_attributes'][adUrlName] == undefined) return;
		if(cfg[FUN_SEARCHBILL + '_attributes'][ifurlName] == undefined) return;
		var url = cfg[FUN_SEARCHBILL + '_attributes'][adUrlName];
		if(url == '') return;

		var ifurl = cfg[FUN_SEARCHBILL + '_attributes'][ifurlName];
		if(ifurl == '') return;
